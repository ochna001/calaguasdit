...
export function LiquidGlassShader({ imageSrc, className = "" }: LiquidGlassShaderProps) {
    ...
    useEffect(() => {
        ...
        // Fixed mouse position (center of the card)
        const mouse = [canvas.width / 2, canvas.height / 2];
        ...
        const handleResize = () => {
            setCanvasSize();
            mouse[0] = canvas.width / 2;
            mouse[1] = canvas.height / 2;
        };
    ...
    return (
        <div className={`relative w-full h-full ${className}`}>
            <canvas
                ref={canvasRef}
                className={`absolute inset-0 w-full h-full rounded-[62px] transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
            />
        </div>
    );
}
